...
import wx.adv
import datetime
import wmm_core.model as WMMv2
import pandas as pd
...
class WMMGui(wx.Frame):
    def __init__(self, parent, title, x, y):
        ...
        # Radio Buttons
        self.rbotN = wx.RadioButton(
            panel, label="North", pos=(50, 10), style=wx.RB_GROUP)
        self.rbotS = wx.RadioButton(panel, label="South", pos=(50, 30))
        self.rbotLat = wx.RadioButton(
            panel, label="West", pos=(50, 90), style=wx.RB_GROUP)
        self.rbotLon = wx.RadioButton(panel, label="East", pos=(50, 110))
        self.rbotKm = wx.RadioButton(
            panel, label="KM", pos=(50, 165), style=wx.RB_GROUP)
    ...
    def writerBook(self, dates):
        ...
        for x in dates:
            ...
            vecHorizontalIntensity = WMMv2.getHorizontalIntensity(
                self.lat, self.lon, x, self.alt)
            vecNorthIntensity = WMMv2.getNorthIntensity(
                self.lat, self.lon, x, self.alt)
            vecEastIntensity = WMMv2.getEastIntensity(
                self.lat, self.lon, x, self.alt)
            vecVerticalIntensity = WMMv2.getVerticalIntensity(
                self.lat, self.lon, x, self.alt)
            vecDeclination = WMMv2.getDeclination(
                self.lat, self.lon, x, self.alt)
            vecDipAngle = WMMv2.getDipAngle(self.lat, self.lon, x, self.alt)
            # Get values from next year
            vecIntensity2 = WMMv2.getIntensity(
                self.lat, self.lon, x+1, self.alt)
            vecHorizontalIntensity2 = WMMv2.getHorizontalIntensity(
                self.lat, self.lon, x+1, self.alt)
            vecNorthIntensity2 = WMMv2.getNorthIntensity(
                self.lat, self.lon, x+1, self.alt)
            vecEastIntensity2 = WMMv2.getEastIntensity(
                self.lat, self.lon, x+1, self.alt)
            vecVerticalIntensity2 = WMMv2.getVerticalIntensity(
                self.lat, self.lon, x+1, self.alt)
            vecDeclination2 = WMMv2.getDeclination(
                self.lat, self.lon, x+1, self.alt)
        ...
        df3.to_excel(writer, sheet_name='Sheet1',
                     startrow=0, startcol=2, index=False)
        df4.to_excel(writer, sheet_name='Sheet1',
                     startrow=0, startcol=3, index=False)
        df5.to_excel(writer, sheet_name='Sheet1',
                     startrow=0, startcol=4, index=False)
        df6.to_excel(writer, sheet_name='Sheet1',
                     startrow=0, startcol=5, index=False)
        df7.to_excel(writer, sheet_name='Sheet1',
                     startrow=0, startcol=6, index=False)
        df8.to_excel(writer, sheet_name='Sheet1',
                     startrow=0, startcol=7, index=False)

        dfd1.to_excel(writer, sheet_name='Sheet1',
                      startrow=0, startcol=9, index=False)
        dfd2.to_excel(writer, sheet_name='Sheet1',
                      startrow=0, startcol=10, index=False)
        dfd3.to_excel(writer, sheet_name='Sheet1',
                      startrow=0, startcol=11, index=False)
        dfd4.to_excel(writer, sheet_name='Sheet1',
                      startrow=0, startcol=12, index=False)
        dfd5.to_excel(writer, sheet_name='Sheet1',
                      startrow=0, startcol=13, index=False)
        dfd6.to_excel(writer, sheet_name='Sheet1',
                      startrow=0, startcol=14, index=False)
        dfd7.to_excel(writer, sheet_name='Sheet1',
                      startrow=0, startcol=15, index=False)
