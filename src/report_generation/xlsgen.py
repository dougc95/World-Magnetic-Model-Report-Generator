...
class WMM:
    ...
    def callWMM(self):
        self.dec = WMMv2.getDeclination(
            self.latitude, self.longitude, self.year, self.altitude)
        self.dip = WMMv2.getDipAngle(
            self.latitude, self.longitude, self.year, self.altitude)
        self.by = WMMv2.getEastIntensity(
            self.latitude, self.longitude, self.year, self.altitude)
        self.bh = WMMv2.getHorizontalIntensity(
            self.latitude, self.longitude, self.year, self.altitude)
        self.ti = WMMv2.getIntensity(
            self.latitude, self.longitude, self.year, self.altitude)
        self.bx = WMMv2.getNorthIntensity(
            self.latitude, self.longitude, self.year, self.altitude)
        self.bz = WMMv2.getVerticalIntensity(
            self.latitude, self.longitude, self.year, self.altitude)
    ...
    def decimalYear(self, year):
        ...
        # Date to day of year
        return date_object.year+(date_object.timetuple().tm_yday/daysInYear)
