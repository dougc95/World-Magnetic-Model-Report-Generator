...
class WorldMagneticModel:
    def __init__(self, data_file_path: str):
        ...
        # Magnetic field components and angles
        self.dec = 0.0  # Declination
        self.dip = 0.0  # Inclination
        self.ti = 0.0   # Total intensity
        self.bx = 0.0   # North component
        self.by = 0.0   # East component
        self.bz = 0.0   # Vertical component
    ...
    def _load_model(self, data_file_path):
        ...
        # The default date might be epoch + 2.5 as per original code (WMM standard)
        self.default_date = self.epoch + 2.5

        # Populate c and cd arrays from data
        # c[m][n], cd[m][n], and if m != 0, also c[n][m-1], cd[n][m-1]
    ...
    def _initialize_model(self):
        ...
        # Convert coefficients to unnormalized form
        for n in range(1, self.maxord + 1):
    ...
    def _calculate_geomag(self, glat, glon, time, alt):
        ...
        # Conversion from geodetic to spherical
        ...
        for n in range(1, self.maxord + 1):
            ...
            for m in range(n+1):
                ...
                if alt != self.oalt or glat != self.olat:
                    if n == m:
                        self.snorm[n + m*13] = st*self.snorm[n-1+(m-1)*13] if (m>0 and n>1) else ct*self.snorm[n-1+m*13]
                        self.dp[m][n] = st*self.dp[m-1][n-1] + ct*self.snorm[n-1+(m-1)*13] if (m>0 and n>1) else ct*self.dp[m][n-1] - st*self.snorm[n-1+m*13]
                    elif n > 1 and n != m:
                        if m > n - 2:
        ...
        # Rotate from spherical to geodetic
        self.bx = -bt*ca - br*sa
        self.by = bp
        self.bz = bt*sa - br*ca
